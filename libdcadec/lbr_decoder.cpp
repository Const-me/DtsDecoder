...
__forceinline void XMScalarSinCosEst( float* pSin, float* pCos, float Value ) noexcept
{
...
// Multiplier to get radians from what was previously an index in sine table
constexpr float sinTab_multiplier = XM_PI / 128;

__forceinline float XMScalarSin( float Value ) noexcept
{
	// Map Value to y in [-pi,pi], x = 2*pi*quotient + remainder.
	float quotient = XM_1DIV2PI * Value;
	if( Value >= 0.0f )
...
static int init_tables( struct lbr_decoder *lbr )
{
	...
	lbr->imdct.reset();
	lbr->imdct = idct_init( lbr, lbr->freq_range + 5, scale );

	/* for( i = 0; i < 256; i++ )
		lbr->sin_tab[ i ] = XMScalarCos( XM_PI * i / 128 ); */

	for( i = 0; i < 16; i++ )
		lbr->lpc_tab[ i ] = XMScalarSin( ( i - 8 ) * ( XM_PI / ( ( i < 8 ) ? 17 : 15 ) ) );
	...
	int group, int group_sf, int synth_idx )
{
	...
	for( int i = 0; i < count; i++ ) {
		...
		if( t->amp[ ch ] )
		{
			...
			float s, c;
			XMScalarSinCosEst( &s, &c, ( t->phs[ ch ] * sinTab_multiplier ) );
			s *= amp;
			c *= amp;
