	...
	int **samples, int nsamples, int *ch_mask )
{
	...
	dca->dmix_sample_buffer.resize( 2 * nsamples );
	...
	samples[ SPEAKER_R ] = dca->dmix_sample_buffer.data() + nsamples;
	*ch_mask = SPEAKER_LAYOUT_STEREO;
	return 1;
}
...
static int alloc_exss_parser( struct dcadec_context *dca )
{
	if( !dca->exss )
	{
		dca->exss = makeUnique<exss_parser>();
		dca->exss->ctx = dca;
	}
	return 0;
}
...
static int alloc_xll_decoder( struct dcadec_context *dca )
{
	if( !dca->xll )
	{
		dca->xll = makeUnique<xll_decoder>();
		dca->xll->ctx = dca;
...
static int alloc_lbr_decoder( struct dcadec_context *dca )
{
	if( !dca->lbr )
	{
		dca->lbr = makeUnique<lbr_decoder>();
		dca->lbr->ctx = dca;
...
DCADEC_API int dcadec_context_parse( struct dcadec_context *dca, uint8_t *data, size_t size )
{
	...
	{
		if( ( ret = core_parse( dca->core, data, (int)size, dca->flags, NULL ) ) < 0 ) {
			dca->core_residual_valid = false;
			return ret;
		}
		if( ret > status )
	...
	if( DCA_MEM32NE( data ) == DCA_32BE_C( SYNC_WORD_EXSS ) ) {
		if( ( ret = alloc_exss_parser( dca ) ) < 0 )
			return ret;
		if( ( ret = exss_parse( dca->exss, data, (int)size ) ) < 0 )
		{
	...
	if( dca->packet & PACKET_EXSS ) {
		...
		{
			if( ( ret = core_parse( dca->core, data, (int)size, dca->flags, asset ) ) < 0 )
			{
				dca->core_residual_valid = false;
				return ret;
			}
...
dcadec_exss_info exss_get_info( struct exss_parser *exss );
dcadec_exss_info core_get_info_exss( struct core_decoder *core );

DCADEC_API std::optional<dcadec_core_info> dcadec_context_get_core_info( struct dcadec_context *dca )
{
	if( dca && ( dca->packet & PACKET_CORE ) )
		return core_get_info( dca->core );
	return std::optional<dcadec_core_info>();
}

DCADEC_API std::optional<dcadec_exss_info> dcadec_context_get_exss_info( struct dcadec_context *dca )
{
	if( dca )
	{
		if( dca->packet & PACKET_EXSS )
	...
	return std::optional<dcadec_exss_info>();
}
