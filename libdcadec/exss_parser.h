...
struct exss_parser
{
	...
	int     exss_index = 0;         ///< Extension substream index
	int     exss_size_nbits = 0;    ///< Number of bits for extension substream size
	int     exss_size = 0;          ///< Number of bytes of extension substream

	bool    static_fields_present = false;  ///< Per stream static fields presence flag
	int     npresents = 0;  ///< Number of defined audio presentations
	int     nassets = 0;    ///< Number of audio assets in extension substream

	bool    mix_metadata_enabled = false;   ///< Mixing metadata enable flag
	int     nmixoutconfigs = 0;         ///< Number of mixing configurations
	std::array<int, 4> nmixoutchs = { 0,0,0,0 };   ///< Speaker layout mask for mixer output channels

	DynamicArray<exss_asset> assets;    ///< Audio asset descriptors
};
...
// dcadec_exss_info exss_get_info( struct exss_parser *exss ) __attribute__( ( cold ) );