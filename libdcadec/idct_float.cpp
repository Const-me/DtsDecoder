...
unique_ptr<idct_context> idct_init( void *parent, int nbits, double scale )
{
	int i, j, k, p, base;

	if( nbits < 2 || nbits > IDCT_BITS )
		return unique_ptr<idct_context>{};

	auto s = makeUnique<idct_context>();
...
template<bool flag>
__forceinline void proc( const struct idct_context *s, double *x )
{
	double y[ IDCT_SIZE / 2 ], tmp;
	int i, j, k, p, q, base;

	const int m = s->nbits - 1;
	const int n = 1 << m;
	const int n2 = n >> 1;

	// for( i = 0; i < n; i++ )
	//	y[ i ] = x[ i ];
	memcpy( y, x, (size_t)1 << ( m + 3 ) );	// sizeof( double ) = 8 = 2 ^ 3, i.e. need 3 more powers of 2 for the count of bytes
...
void imdct_fast( const struct idct_context *s, const float *input, float *output )
{
	...
	for( i = 0; i < n4; i++ )
	{
		output[ n4 + i ] = (float)( -a[ i ] * s->as[ i ] + b[ i ] * s->ac[ i ] );
		output[ n - n4 - i - 1 ] = (float)( -a[ i ] * s->ac[ i ] - b[ i ] * s->as[ i ] );
	}
