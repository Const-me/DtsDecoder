...
{
	memset( nsubbands, 0, sizeof( nsubbands ) );
	memset( subband_vq_start, 0, sizeof( subband_vq_start ) );
	memset( joint_intensity_index, 0, sizeof( joint_intensity_index ) );
	memset( transition_mode_sel, 0, sizeof( transition_mode_sel ) );
	memset( scale_factor_sel, 0, sizeof( scale_factor_sel ) );
	memset( bit_allocation_sel, 0, sizeof( bit_allocation_sel ) );
	memset( quant_index_sel, 0, sizeof( quant_index_sel ) );
	memset( scale_factor_adj, 0, sizeof( scale_factor_adj ) );

	memset( nsubsubframes, 0, sizeof( nsubsubframes ) );
	memset( prediction_mode, 0, sizeof( prediction_mode ) );
	memset( prediction_vq_index, 0, sizeof( prediction_vq_index ) );
	memset( bit_allocation, 0, sizeof( bit_allocation ) );
	memset( transition_mode, 0, sizeof( transition_mode ) );
	memset( scale_factors, 0, sizeof( scale_factors ) );
	memset( joint_scale_sel, 0, sizeof( joint_scale_sel ) );
	memset( joint_scale_factors, 0, sizeof( joint_scale_factors ) );

	memset( prim_dmix_coeff, 0, sizeof( prim_dmix_coeff ) );

	memset( xxch_dmix_mask, 0, sizeof( xxch_dmix_mask ) );
	memset( xxch_dmix_coeff, 0, sizeof( xxch_dmix_coeff ) );
	memset( x96_subband_samples, 0, sizeof( x96_subband_samples ) );
	memset( subband_samples, 0, sizeof( subband_samples ) );
	...
	int scale, bool residual )
{
	...
	if( step_scale > ( 1 << 23 ) ) {
		shift = 32 - dca_clz( (uint32_t)( step_scale >> 23 ) );
		step_scale >>= shift;
	}
...
static int alloc_sample_buffer( struct core_decoder *core )
{
	...
	if( core->subband_buffer.resize( nframesamples + nlfesamples ) )
	{
		int* subband_buffer = core->subband_buffer;
		for( int ch = 0; ch < MAX_CHANNELS; ch++ )
			for( int band = 0; band < MAX_SUBBANDS; band++ )
				core->subband_samples[ ch ][ band ] = subband_buffer + ( ch * MAX_SUBBANDS + band ) * nchsamples + NUM_ADPCM_COEFFS;
		core->lfe_samples = subband_buffer + nframesamples;
	}
...
int core_filter( struct core_decoder *core, int flags )
{
	...
	const int synth_x96 = ( flags & DCADEC_FLAG_CORE_SYNTH_X96 ) ? 1 : 0;
	...
	// if (ta_zalloc_fast(core, &core->output_buffer, core->npcmsamples * dca_popcount(core->ch_mask), sizeof(int)) < 0)
	 //   return -DCADEC_ENOMEM;
	core->output_buffer.resize( core->npcmsamples * dca_popcount( core->ch_mask ) );
	...
	if( !core->subband_dsp_idct[ synth_x96 ] )
		core->subband_dsp_idct[ synth_x96 ] = idct_init( core, 5 + synth_x96, 0.25 );

	// Filter primary channels
	for( int ch = 0; ch < core->nchannels; ch++ ) {
		// Allocate subband DSP
		if( !core->subband_dsp[ ch ] )
			core->subband_dsp[ ch ].create( core->subband_dsp_idct[ synth_x96 ], flags );
...
static int alloc_x96_sample_buffer( struct core_decoder *core )
{
	...
	// Reallocate subband sample buffer
	if( core->x96_subband_buffer.resize( nframesamples ) )
	{
		for( int ch = 0; ch < MAX_CHANNELS; ch++ )
			for( int band = 0; band < MAX_SUBBANDS_X96; band++ )
				core->x96_subband_samples[ ch ][ band ] = core->x96_subband_buffer.data() +
...
void core_clear( struct core_decoder *core )
{
	if( core )
	{
		...
		if( core->x96_subband_buffer )
			erase_x96_adpcm_history( core );
		for( int ch = 0; ch < MAX_CHANNELS; ch++ )
			core->subband_dsp[ ch ].clear();
		core->output_history_lfe = 0;
	}
}

dcadec_core_info core_get_info( struct core_decoder *core )
{
	dcadec_core_info info;
...
dcadec_exss_info core_get_info_exss( struct core_decoder *core )
{
	dcadec_exss_info info;
	info.nchannels = core->nchannels + !!core->lfe_present;
	info.sample_rate = core->sample_rate << ( ( core->ext_audio_mask & CSS_X96 ) ? 1 : 0 );
