...
static int parse_dmix_coeffs( struct xll_chset *chs )
{
	...
	chs->dmix_coeff.resize( m * n * 2 );

	// Setup buffer pointers for current and previous frame
	bool valid = ( chs->dmix_coeffs_signature == XLL_DMIX_SIGNATURE( chs ) );
	chs->dmix_coeff_cur = chs->dmix_coeff.data() + m * n * chs->dmix_coeffs_parity;
	chs->dmix_coeff_pre = chs->dmix_coeff.data() + m * n * ( chs->dmix_coeffs_parity ^ valid );
...
static int chs_alloc_msb_band_data( struct xll_chset *chs )
{
	...
	chs->sample_buffer1.resize( ( xll->nframesamples + XLL_DECI_HISTORY ) * chs->nchannels * chs->nfreqbands );

	int *ptr = chs->sample_buffer1.data() + XLL_DECI_HISTORY;
...
static int chs_alloc_lsb_band_data( struct xll_chset *chs )
{
	...
	chs->sample_buffer2.resize( xll->nframesamples * chs->nchannels * nfreqbands );
	...
	for( int band = 0; band < chs->nfreqbands; band++ )
	{
		...
		{
			for( int i = 0; i < chs->nchannels; i++ )
				chs->bands[ band ].lsb_sample_buffer[ i ] = nullptr;
		}
	}
...
static int chs_assemble_freq_bands( struct xll_chset *chs )
{
	...
	chs->sample_buffer3.resize( 2 * nsamples * chs->nchannels );
...
static int copy_to_pbr( struct xll_decoder *xll, uint8_t *data, int size, int delay )
{
	...
	xll->pbr_buffer.resize( XLL_PBR_SIZE + DCADEC_BUFFER_PADDING );
...
static int parse_frame_pbr( struct xll_decoder *xll, uint8_t *data, int size, struct exss_asset *asset )
{
	...
	memcpy( xll->pbr_buffer.data() + xll->pbr_length, data, size );
	xll->pbr_length += size;
	...
	if( xll->frame_size == xll->pbr_length ) {
		...
		memmove( xll->pbr_buffer, xll->pbr_buffer.data() + xll->frame_size, xll->pbr_length );
	}
