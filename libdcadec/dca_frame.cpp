...
static void swapBytes( uint16_t* dest, size_t count, const uint16_t* src )
{
	const uint16_t* const srcEnd = src + count;
	const uint16_t* const srcEndAligned = src + ( count / 8 ) * 8;
#ifdef _MSC_VER
	const __m128i shuf = _mm_setr_epi8( 1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14 );
	for( ; src < srcEndAligned; src += 8, dest += 8 )
	{
	...
	for( ; src < srcEndAligned; src += 8, dest += 8 )
	{
		...
		val = vrev16q_u8( val );
		vst1q_u8( (uint8_t*)dest, val );
	}
#endif
	for( ; src < srcEnd; src++, dest++ )
		*dest = dca_bswap16( *src );
}
	...
	const uint8_t *src, size_t src_size )
{
	...
	case DCA_32BE_C( SYNC_WORD_EXSS_LE ):
		count = ( src_size + 1 ) / 2;
		swapBytes( _dst, count, (const uint16_t*)_src );
		*dst_size = src_size;
...
DCADEC_API int dcadec_frame_parse_header( const uint8_t *data, size_t *size )
{
	...
	bits_init( &bits, header, (int)header_size );
...
{
	size_t padding = (size_t)( -(ptrdiff_t)size & ( DCADEC_FRAME_BUFFER_ALIGN - 1 ) );
