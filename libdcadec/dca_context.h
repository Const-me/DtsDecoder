...
struct dcadec_context
{
	dcadec_log_cb   log_cb = nullptr;         ///< Logging callback function
	void            *log_cbarg = nullptr;     ///< Logging callback argument
	int             log_shown = 0;      ///< Bitmask of logging levels shown once

	int     flags = 0;  ///< Context flags passed to dcadec_context_create()
	int     packet = 0; ///< Packet flags set by dcadec_context_parse()

	struct core_decoder *core = nullptr;  ///< Core decoder context
	unique_ptr<exss_parser> exss;  ///< EXSS parser context
	unique_ptr<xll_decoder> xll;   ///< XLL decoder context
	unique_ptr<lbr_decoder> lbr;   ///< LBR decoder context

	bool    has_residual_encoded = false;   ///< XLL residual encoded channels present
	bool    core_residual_valid = false;    ///< Core valid for residual decoding

	DynamicArray<int> dmix_sample_buffer;    ///< Primary channel set downmixing buffer

	int     status = 0;             ///< Filtering status
	int     nframesamples = 0;      ///< Number of PCM samples per channel
	int     sample_rate = 0;        ///< Sample rate in Hz
	int     bits_per_sample = 0;    ///< PCM resolution in bits
	int     profile = 0;            ///< Type of DTS profile decoded
	int     channel_mask = 0;       ///< Channel or speaker mask
	std::array<int*, SPEAKER_COUNT> samples; ///< Sample buffer pointers
};
...
struct dcadec_core_info {
	...
	int     ext_audio_type;     /**< Extended audio type (only meaningful when ext_audio_present is true) */
};
 ...
 * @param dca   Pointer to decoder context.
 *
 * @return      DTS core information structure
 */
std::optional<dcadec_core_info> dcadec_context_get_core_info( dcadec_context *dca );
 ...
 * @param dca   Pointer to decoder context.
 *
 * @return      EXSS information structure
 */
std::optional<dcadec_exss_info> dcadec_context_get_exss_info( dcadec_context *dca );
